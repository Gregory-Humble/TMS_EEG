...
SaveDir = 'E:\Alz_Clinical_Trial\Alz_Data_Analysis_10JAN20\TMSEEG\TEPoutput_ALZ_MODERATE';
mkdir(SaveDir);
time = [];
inPath = [];
time = {'BL'};
hemi = {'leftpfc'};
IDlist = {'301', '305', '307', '312', '314', '318', '326', '328', '330', '341', '343', '346'};
...
for h = 1:length(hemi);
    for t = 1:size(time,1);
        ...
        %groupdata = [];
            filename = [ IDlist{ID},'_TMSEEG_', time{t,1}, '_', hemi{h}, '_ds_ica1_filt_ica2_clean_reref.set'];%
            ...
            TEPdata(:,1,ID) = mean(subjav(:,tp1:tp2),2); % 1 -> TEPdata = [all chans x column1 ( = N100) x ID]
            %   TEPdata_ROIlabels(i,1)=i; %char2str(shortname)
            TEPdata_ROIlabels{ID,1} = shortname;
            TEPdata_ROI(ID,1) = mean(subjav(IND,tp1:tp2),2); %N100 for each ID at channel 8
            
            
            %P60
            tp1 = find(EEG.times == 45);
            tp2 = find(EEG.times == 75);
            TEPdata(:,2,ID) = mean(subjav(:,tp1:tp2),2);
            TEPdata_ROI(ID,2) = mean(subjav(IND,tp1:tp2),2); %P200 for each ID at channel 8
            
            %N100
            tp1 = find(EEG.times == 85);
            tp2 = find(EEG.times == 145);
            TEPdata(:,3,ID) = mean(subjav(:,tp1:tp2),2);
            TEPdata_ROI(ID,3) = mean(subjav(IND,tp1:tp2),2);
            
            %P200
            tp1 = find(EEG.times == 170);
            tp2 = find(EEG.times == 230);
            TEPdata(:,4,ID) = mean(subjav(:,tp1:tp2),2);
            TEPdata_ROI(ID,4) = mean(subjav(IND,tp1:tp2),2);
            ...
            TEPdata_GMFA(ID,:) = subGMFA(1,tp1:tp2);
            
            %% save TEP data for each ID & TOI
            
            path = pwd;
            cd(DataDir);
            fileID = fopen('TEPdata_ROI_ALZHEIMERS_MODERATE.txt', 'a');
            %fprintf(fileID, '%s %f %f \n', shortname, TEPdata_ROI(i,2), TEPdata_ROI(i,3));
            fprintf(fileID, '%s %f %f %f %f %f \n', shortname, TEPdata_ROI(ID,2), TEPdata_ROI(ID,3), TEPdata_ROI(ID,4));
            fclose(fileID);
            cd(path);
...
%% END OF INDIVIDUAL DATA ANALYIS
        ...
        %% GROUP TOPOPLOTS - AT PRECISE LATENCIES 40 60 100 200
            ...
            cd(SaveDir)
            saveas(gcf, 'Group_Topoplot_Precise_Latencies_BL_leftpfc_ALZHEIMERS_MODERATE.png');
            saveas(gcf, 'Group_Topoplot_Precise_Latencies_BL_leftpfc_ALZHEIMERS_MODERATE.fig');
        ...
        set(gcf, 'Position',  [5, 5, 1500, 900]);
        cd(SaveDir);
        saveas(gcf, 'Group_Topoplot_Within_Time_Ranges_BL_leftpfc_ALZHEIMERS_MODERATE.png');
        saveas(gcf, 'Group_Topoplot_Within_Time_Ranges_BL_leftpfc_ALZHEIMERS_MODERATE.fig');
        
        %thisfilename = ['TEPdata2plot_',time{t,1},'_',hemi{h},'.mat'];
        thisfilename = ['TEPdata2plot_',time{t,1},'_',hemi{h}, '_ALZHEIMERS_MODERATE.mat'];
        ...
        save(['Topoplot_allChan_x_TOI_',time{t,1},'_',hemi{h}, '_ALZHEIMERS_MODERATE.mat'], 'groupaverage'); %this is [chan x toi]
...
ch = 6 % 6 = F1
for c = ch; %1:50
    results = ['TEPdata2plot_', time{t,1}, '_', hemi{h}, '_ALZHEIMERS_MODERATE.mat'];
    load (results);
    ...
    hold on
    legend('Alzheimers MODERATE',['95%CI'],'Location','bestoutside')%,r, b)
    % legend('off')
end
...
%P200
txt = 'P200';
thistext = text(200,2.1,txt,'FontSize',11);
thistext.HorizontalAlignment = 'center'

%Fixes legend properties to exclude random data legend entries
legend('Alzheimers MODERATE',['95%CI'],'Location','bestoutside');
set(gcf, 'Position',  [5, 5, 1500, 900]);
cd (SaveDir);
saveas(gcf, 'TEP_F1_BL_leftpfc_ALZHEIMERS_MODERATE.png');
saveas(gcf, 'TEP_F1_BL_leftpfc_ALZHEIMERS_MODERATE.fig');
...
figure;
    ...
    set(gcf, 'Position',  [5, 5, 1500, 900]);
    cd (SaveDir);
    saveas(gcf, 'Grand_Average_ButterflyPlotTEP_F1_BL_leftpfc_ALZHEIMERS_MODERATE.png');
    saveas(gcf, 'Grand_Average_ButterflyPlotTEP_F1_BL_leftpfc_ALZHEIMERS_MODERATE.fig');