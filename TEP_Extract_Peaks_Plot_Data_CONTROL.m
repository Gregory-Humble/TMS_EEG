...
clearvars;
eeglab;
DataDir = 'E:\Alz_Clinical_Trial\Alz_Data_Analysis_10JAN20\TMSEEG\TMSEEG_CONTROL_clean_2\BL';
SaveDir = 'E:\Alz_Clinical_Trial\Alz_Data_Analysis_10JAN20\TMSEEG\TEPoutput_CONTROL';
mkdir(SaveDir);
time = [];
inPath = [];
time = {'BL'};
...
for h = 1:length(hemi);
    for t = 1:size(time,1);
        ...
        for namei = 1:numel(names);
            ...
            path = pwd;
            cd(DataDir);
            fileID = fopen('TEPdata_ROI_CONTROL.txt', 'a');
        ...
        %% GROUP TOPOPLOTS - AT PRECISE LATENCIES 40 60 100 200
            ...
            cd(SaveDir)
            saveas(gcf, 'Group_Topoplot_Precise_Latencies_BL_leftpfc_CONTROL.png');
            saveas(gcf, 'Group_Topoplot_Precise_Latencies_BL_leftpfc_CONTROL.fig');
        ...
        set(gcf, 'Position',  [5, 5, 1500, 900]);
        cd(SaveDir);
        saveas(gcf, 'Group_Topoplot_Within_Time_Ranges_BL_leftpfc_CONTROL.png');
        saveas(gcf, 'Group_Topoplot_Within_Time_Ranges_BL_leftpfc_CONTROL.fig');
        thisfilename = ['TEPdata2plot_',time{t,1},'_',hemi{h}, '_CONTROL.mat'];
        save(thisfilename, 'groupdata'); %this is [chan x time points x ID] (51 x 3000 x 10)
        save(['Topoplot_allChan_x_TOI_',time{t,1},'_',hemi{h}, '_CONTROL.mat'], 'groupaverage'); %this is [chan x toi]
        groupaverage = [];        
...
for c = ch; 
    ...
    hold on
    legend('Controls',['95%CI'],'Location','bestoutside')%,r, b)
    % legend('off')
end
...
legend('Controls',['95%CI'],'Location','bestoutside');

cd (SaveDir);
set(gcf, 'Position',  [5, 5, 1500, 900]);
saveas(gcf, 'TEP_F1_BL_leftpfc_CONTROL.png');
saveas(gcf, 'TEP_F1_BL_leftpfc_CONTROL.fig');
...
figure;
    ...
    %N40
    txt = 'N40';
    thistext = text(40,-2.1,txt,'FontSize',11, 'Color', 'b');
    thistext.HorizontalAlignment = 'center';

    %P60
    txt = 'P60';
    thistext = text(60,2.0,txt,'FontSize',11, 'Color', 'b');
    thistext.HorizontalAlignment = 'center';

    %N100
    txt = 'N100';
    thistext = text(120,-3.8,txt,'FontSize',11, 'Color', 'b');
    thistext.HorizontalAlignment = 'center';

    %P200
    txt = 'P200';
    thistext = text(220,4.0,txt,'FontSize',11, 'Color', 'b');
    thistext.HorizontalAlignment = 'center';

    %plot the group averaged GMFA offset by 4.5
    newyvector = [-100:500];
    p = plot(newyvector,mean(TEPdata_GMFA,1)-7.5);
    ...
    set(gca, 'ylim', [-7 5]);
    cd (SaveDir);
    saveas(gcf, 'Grand_Average_ButterflyPlotTEP_F1_BL_leftpfc_CONTROL.png');
    saveas(gcf, 'Grand_Average_ButterflyPlotTEP_F1_BL_leftpfc_CONTROL.fig');
