...
DataDirALZMILD = 'E:\Alz_Clinical_Trial\Alz_Data_Analysis_10JAN20\TMSEEG\TMSEEG_ALZ_clean_2\BL\leftpfc';
SaveDirALZMILD = 'E:\Alz_Clinical_Trial\Alz_Data_Analysis_10JAN20\TMSEEG\TEPoutput_ALZ_MILD';
DataDirALZMODERATE = 'E:\Alz_Clinical_Trial\Alz_Data_Analysis_10JAN20\TMSEEG\TMSEEG_ALZ_clean_2\BL\leftpfc';
SaveDirALZMODERATE = 'E:\Alz_Clinical_Trial\Alz_Data_Analysis_10JAN20\TMSEEG\TEPoutput_ALZ_MODERATE';
SaveDir =  'E:\Alz_Clinical_Trial\Alz_Data_Analysis_10JAN20\TMSEEG\TEPoutput_GROUP_COMPARISONS_MILD_MODERATE';
inPath = [];
time = {'BL'};
hemi = {'leftpfc'};
%% GROUP TEP
...
for c = ch; 
    ...
    EEG = pop_loadset('filename','101_TMSEEG_BL_leftpfc_ds_ica1_filt_ica2_clean_reref.set','filepath','E:\\Alz_Clinical_Trial\\Alz_Data_Analysis_10JAN20\\TMSEEG\\TMSEEG_CONTROL_clean_2\\BL\\');
...
for c = ch; 
    ...
    pre_groupchandata = groupdata(c,:,:);
    data = pre_groupchandata;
    linecolour = 'y'; shadingalpha = 0.3; %1/50;
    M = mean(data,3);
...
%% Alzheiemers Moderate
time = {'BL'};
hemi = {'leftpfc'};
ch = 6; % 6 = F1 for ALZHEIEMRS
cd(SaveDirALZMODERATE);
for h = 1:length(hemi);
    for t = 1:size(time,1);
for c = ch; 
    ...
    drawnow;
    % 95%CI
    legend('Controls',['95%CI'],'Alzheimers mild',['95%CI'],'Alzheimers mmoderate',['95%CI'],'Location','bestoutside');   
...
rectangle('Position', [85, -5, 60, 10], ...
    'FaceColor', [0, 0, 0, 0.05], ...
    'EdgeColor', [0, 0, 0, 0.0]);
rectangle('Position', [170, -5, 60, 10], ...
    'FaceColor', [0, 0, 0, 0.05], ...
    'EdgeColor', [0, 0, 0, 0.0]);
...
%P200
txt = 'P200';
thistext = text(200,4.5,txt,'FontSize',11);
thistext.HorizontalAlignment = 'center'

%Fixes legend properties to exclude random data legend entries
legend('Controls',['95%CI'],'Alzheimers mild',['95%CI'],'Alzheimers moderate',['95%CI'],'Location','bestoutside');  

cd (SaveDir);
set(gcf, 'Position',  [5, 5, 1500, 900]);
saveas(gcf, 'TEP_F1_BL_leftpfc_CONTROL_MILD_MODERATE.png');
saveas(gcf, 'TEP_F1_BL_leftpfc_CONTROL_MILD_MODERATE.fig');