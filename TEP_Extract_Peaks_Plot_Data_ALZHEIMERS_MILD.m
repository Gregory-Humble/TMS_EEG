...
SaveDir = 'E:\Alz_Clinical_Trial\Alz_Data_Analysis_10JAN20\TMSEEG\TEPoutput_ALZ_MILD';
mkdir(SaveDir);
time = [];
inPath = [];
time = {'BL'};
hemi = {'leftpfc'};
IDlist = {'302', '306', '308', '309', '310', '311', '315', '317', '319', '320', '322', '324', '325', '327', '329', '333', '335', '336', '338', '345', '347', '348', '351'};
...
for h = 1:length(hemi);
    for t = 1:size(time,1);
        ...
        %groupdata = [];
            ...
            path = pwd;
            cd(DataDir);
            fileID = fopen('TEPdata_ROI_ALZHEIMERS_MILD.txt', 'a');
...
%% END OF INDIVIDUAL DATA ANALYIS
        ...
        %% GROUP TOPOPLOTS - AT PRECISE LATENCIES 40 60 100 200
            ...
            cd(SaveDir)
            saveas(gcf, 'Group_Topoplot_Precise_Latencies_BL_leftpfc_ALZHEIMERS_MILD.png');
            saveas(gcf, 'Group_Topoplot_Precise_Latencies_BL_leftpfc_ALZHEIMERS_MILD.fig');
        ...
        set(gcf, 'Position',  [5, 5, 1500, 900]);
        cd(SaveDir);
        saveas(gcf, 'Group_Topoplot_Within_Time_Ranges_BL_leftpfc_ALZHEIMERS_MILD.png');
        saveas(gcf, 'Group_Topoplot_Within_Time_Ranges_BL_leftpfc_ALZHEIMERS_MILD.fig');
        
        %thisfilename = ['TEPdata2plot_',time{t,1},'_',hemi{h},'.mat'];
        thisfilename = ['TEPdata2plot_',time{t,1},'_',hemi{h}, '_ALZHEIMERS_MILD.mat'];
        ...
        save(['Topoplot_allChan_x_TOI_',time{t,1},'_',hemi{h}, '_ALZHEIMERS_MILD.mat'], 'groupaverage'); %this is [chan x toi]
...
ch = 6 % 6 = F1
for c = ch; %1:50
    results = ['TEPdata2plot_', time{t,1}, '_', hemi{h}, '_ALZHEIMERS_MILD.mat'];
    load (results);
    ...
    hold on
    legend('Alzheimers MILD',['95%CI'],'Location','bestoutside')%,r, b)
    % legend('off')
end
...
legend('Alzheimers MILD',['95%CI'],'Location','bestoutside');
set(gcf, 'Position',  [5, 5, 1500, 900]);
cd (SaveDir);
saveas(gcf, 'TEP_F1_BL_leftpfc_ALZHEIMERS_MILD.png');
saveas(gcf, 'TEP_F1_BL_leftpfc_ALZHEIMERS_MILD.fig');
...
figure;
    ...
    set(gcf, 'Position',  [5, 5, 1500, 900]);
    cd (SaveDir);
    saveas(gcf, 'Grand_Average_ButterflyPlotTEP_F1_BL_leftpfc_ALZHEIMERS_MILD.png');
    saveas(gcf, 'Grand_Average_ButterflyPlotTEP_F1_BL_leftpfc_ALZHEIMERS_MILD.fig');